...
function searchData(query) {
    ...
    const matches = database.filter(person => 
        ...
        (person.nationalNumber && person.nationalNumber.includes(query)) ||
...
function displayResults(data) {
    ...
    results.innerHTML = data.map((person, index) => `
        <div class="card" id="card-${index}">
            ...
            <div class="actions">
                <button onclick="copyText('${person.fullName} — وطني: ${person.nationalNumber || 'لا يوجد'} — هاتف: ${person.phone || 'لا يوجد'}')">📋 نسخ</button>
...
function generateQR(index, dataStr) {
    try {
        ...
        const qrData = `${person.fullName} | وطني: ${person.nationalNumber || 'لا يوجد'} | هاتف: ${person.phone || 'لا يوجد'}`;
